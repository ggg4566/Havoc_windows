...
func (b *Builder) Build() bool {
    ...
    if sysType == "windows"{
    	b.CompileDir = utils.GetTeamserverPath()+"\\tmp\\" + utils.GenerateID(10) + "\\"
    } else {
	...
	if b.config.Arch == ARCHITECTURE_X64 {
		logger.Debug(b.compilerOptions.Config.Compiler64);
...
func (b *Builder) Cmd(cmd string) bool {
	var (
		...
		Command_linux = exec.Command("sh", "-c", cmd)
		stdout  bytes.Buffer
		stderr  bytes.Buffer
		err     error
	)
	...
	if runtime.GOOS=="windows"{
		...
		Command_linux.Dir = b.sourcePath
		Command_linux.Stdout = &stdout
		Command_linux.Stderr = &stderr
		err = Command_linux.Run()
	}
